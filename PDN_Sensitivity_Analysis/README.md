...
- **rail / net（VDD / GND）**：每個 physical layer 上有兩張網：**VDD rail** + **GND rail**。  
  ...
  - 例子：`rows=73, cols=73, layers=1` → unknowns = `2*1*73*73 = 10658`（你在輸出也看得到 Nodes: 10658）。
...
入口程式：`src/examples/pdn_load_example.cpp`
...
8. **輸出**
   ...
   - `out_voltspot/pdn_with_loads_branches*.csv`：給可視化用
...
那通常就是 **pad 分佈**：pad 附近電壓被拉住，**IR drop 最小**。  
如果你用 `hot` colormap，**低值會比較黑/深色**，所以 pads 會很顯眼。

---

### 為什麼最大值會跟正解差很多（例如你看過 0.9% vs 0.55%）？

通常是「條件不一致」：

- **pad 分佈模式不一致**：`PDN_padconfig=1`（補滿所有 pad seats）vs `PDN_padconfig=0`（用稀疏 padloc）
- **padR 不一致**：VoltSpot config 裡 `PDN_padR=10e-3`，若你用不同值，供電“硬/軟”就變了

本專案現在會依 `pdn.config` 的 `PDN_padconfig/PDN_padR` 跑，因此更接近 VoltSpot。

---

### Solver 在做什麼？（大 KCL，用「實際數學」算一遍）

你可以把 solver 想成：對每個 node 寫 KCL，最後變成一個超大的線性方程 \(Gx=b\)。

下面用一個最小可算的例子（**只看 VDD rail、兩個 node**）示範「矩陣一列」是怎麼來的：

- node0 ↔ node1 有電阻 \(R=1\Omega \Rightarrow g=1/R=1\text{ S}\)
- node0 是 VDD pad，pad 電阻 \(R_p=0.5\Omega \Rightarrow g_p=1/R_p=2\text{ S}\)
- 供應電壓 \(V_{dd}=1.0\text{ V}\)
- node1 有負載電流 \(I_{load}=0.2\text{ A}\)（VoltSpot 模型：VDD RHS = \(-I_{load}\)）
...
- **對角線**：鄰居電導總和 +（pad 的 \(1/R_p\)）
- **非對角線**：跟鄰居連線的 \(-g\)
- **RHS**：VDD rail 放 \(-I_{load}\)；pad node 另外加 \(V_{dd}/R_p\)
...
- **`out_voltspot/pdn_with_loads_branches.csv`**：branch（電阻邊）
